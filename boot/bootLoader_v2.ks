...
global function copyLib
{
    parameter srcFile.

    local destFile to path("data_0:/lib/" + srcFile).
    set srcFile to path("0:/lib/" + srcFile).
...
global function runOnceLocal
{
    parameter runFile.

    if addons:rt:hasKscConnection(ship)
    {
        runOncePath(runFile:toString:replace(runFile:root:toString, "0:/")).
    }
    else
    {
        runOncePath(runFile).
