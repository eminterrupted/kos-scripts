...
// INPUTS
// =================================================
	set fPath to "0:/data/ksptot/{0}_{1}.csv":format(Ship:Name:Replace(" ","_"), TimeStamp(time:seconds):Calendar). // "bigLauncher1.csv". //change "test.csv" to your CSV file name.  See step (4) above.

	set printOutput to true. //set to false to disable output display (time, steering, orbit, etc)

    if _params:Length > 0
    {
        set fPath to _params[0].
        if _params:Length > 1 set printOutput to _params[1].
    }
...
runOncePath("0:/lib/kslib/lib_num_to_formatted_str.ks").
runOncePath("0:/lib/kslib/lib_navball.ks").

local cachedIPU to Config:IPU.
local cachedSas to SAS.
