...
from { local i to 0. local doneFlag to false.} until i = CoreTagRef:Regex:Cat:Values:Length or doneFlag step { set i to i + 1.} do 
{
    local regex to CoreTagRef:Regex:Cat:Values[i].
    if mp:Cat:MatchesPattern(regex)
    {
        set mpType to CoreTagRef:Regex:Cat:Keys[i].
...
{
    runPath("0:/_main/launch/downrangerLaunch.ks", list(stgLim, launchHdg, launchAng)).
}