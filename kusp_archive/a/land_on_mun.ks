...
until ship:altitude <= 12500 or alt:radar <= 7500 
{
...
if ship:partstaggedpattern("onTouchdown"):length > 0 
{
    for p in ship:partsTaggedPattern("onTouchdown")
    {
        if p:tag:contains("solar") activate_solar(p).
        else if p:tag:contains("comm") activate_antenna(p).
    }
}
