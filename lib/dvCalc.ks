...
// #region
    ...
    global function BurnDurStage
    {
        ...
        {
            local stgEngs to choose g_ShipEngines:IGNSTG[key]:ENG if g_ShipEngines:IGNSTG:HasKey(key) else list().
            if stgEngs:Length > 0 
            {
                local engSpecs to GetEnginesSpecs(stgEngs):IGNSTG[key].
                local exhVel to GetExhVel(stgEngs).
                // writeJson(stgSpecs, "0:/test/stgSpecs.json").

                local stgThr to engSpecs:STGMAXTHRUST.
