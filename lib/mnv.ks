...
// #region
    ...
    global function ExecNodeBurn
    {
        ...
        else
        {
            ...
            if g_ActiveEngines_Specs:AllowRestart
            {
                if g_ActiveEngines_Specs:Ignitions = 0
                {
                    set useNext to True.
                }
                //else if g_ActiveEngines_Spec:EstBurnTime <= g_ActiveEngines_Spec:SpoolTime + 0.25 
                else if g_ActiveEngines_Specs:BurnTimeRemaining <= g_ActiveEngines_Specs:SpoolTime + 0.25 
                {
                    set useNext to True.
                }
            }
            ...
            else
            {
                ...
                set burnEngsSpec to g_ActiveEngines_Specs.
            }
            ...
            local softShutdownDV to max(0.01, dvRate * (g_ActiveEngines_Specs:SpoolTime * 0.2)).
            ...
            {   
                ...
                if g_ActiveEngines_Specs:SpoolTime > 0.1
                {
                    set softShutdownDV to dvRate * (g_ActiveEngines_Specs:SpoolTime * 0.1).
                }
    ...
    global function ExecNodeBurn_Next
    {
        ...
        else
        {
            ...
            for eng in g_ActiveEngines
            {
                ...
                set g_ActiveSpecs:SPOOLTIME to max(g_ActiveSpecs:SPOOLTIME, g_ShipEngines_Specs[eng:Stage]:EngSpecs[eng:UID]:SPOOLTIME).
                g_ActiveSpecs:Add(eng:UID, engSpec).
            }
            set g_ActiveEngines_Data to GetEnginesPerformanceData(g_ActiveEngines).
    ...
    // ExecNodeBurn_NextNext :: _inNode<node> -> <none>
    // Given a node object, executes it
    // Variant to focus on engine cutoff timing
    global function ExecNodeBurn_NextNext
    {
        parameter _inNode is node(),
                  _stageLimit is 0.
        ...
        else
        {
            ...
            local burnEngs to list().
            local burnEngsSpec to lexicon().
            local fullBurnDuration to 0.
            local halfBurnDuration to 0.
            local settleProgress to 0.
            local ullageExecTS to 0.
            local ullageFlag to false.
            local warpFlag to False.
            ...
            local burnLeadTS     to 0.
            local burnTS         to 0.
            local preSpinArmed   to false.
            local spinFactor     to 1.
            local spinCutoffTS   to Time:Seconds + 99999999.
            local spinStabDurSecs to 0. // Relative to burnTS
            local spinStabLeadSecs to 0. // Relative to burnTS
            local spinTS         to Time:Seconds + 999999.
            // local throttleHelper to { parameter _throt is 0. if _throt <= 0 return 0. else return 1.}.
            local ullageLeadSecs to 18.
            local ullageTS       to 0.
            local warpLeadSecs   to 3.
            local warpTS         to 0.
            ...
            // Calculate Burn Times
            local burnDurations to GetBurnDurations(dv).
            set fullBurnDuration to burnDurations[0].
            set halfBurnDuration to burnDurations[3].
            
            set burnTS to _inNode:Time - halfBurnDuration.
            ...
            if g_ActiveSpecs:AllowRestart
            {
                ...
                else if g_ActiveSpecs:BurnTimeRemaining <= g_ActiveSpecs:SpoolTime * 1.125
                {
                    set useNext to True.
                }
            }
            ...
            // Initiate the burn with enough time for engine spoolup to occur.
            set burnTS to Round(burnTS - (burnEngsSpec:spooltime * 1.1), 3). // This allows for spool time + adds a bit of buffer

            // If ullage is needed, calcualate that TS here
            if burnEngsSpec:ULLAGE
            {
                set ullageFlag to true.
                set burnUllageTime to Max(6, Round(ullageLeadSecs * (1 - burnEngs[0]:FuelStability), 3)).
                set ullageTS to burnTS - burnUllageTime.
            }

            // if burnEngsSpec:ThrottleEnabled:Length > 0
            // {
            //     set throttleHelper to { parameter _throt is 0. if _throt <= 0 { return 0.} else { return Min(1, Max(-1, _throt)).} }.
            // }
            ...
            local preSpin to 0.         
            local preSpinSecs to 0.
            for p in Ship:PartsTaggedPattern("SpinDC\|\d+")
            {
                OutInfo("SpinDC Found").
                local pSplit to p:Tag:Split("|").
                local spinDCParam to pSplit[1]:ToNumber(12).
                if pSplit:Length > 2
                {
                    set spinFactor to Max(-1, Min(1, pSplit[2]:ToNumber(1))).
                }
                // OutDebug(fullBurnDuration + " | " + burnEngsSpec:BurnTimeRemaining, 2).

                set spinStabDurSecs to spinDCParam.
                set spinStabLeadSecs to 0.

                if p:Stage < Stage:Number - 1
                {   
                    from { local i to Stage:Number.} until i <= p:Stage step { set i to i - 1.} do 
                    {
                        if burnDurations:Stage:HasKey(i)
                        {
                            set spinStabLeadSecs to spinStabLeadSecs + burnDurations:Stage[i].
                        }
                    }
                    if spinStabLeadSecs - spinDCParam < 0
                    {
                        set preSpinSecs to spinStabLeadSecs - spinDCParam.
                    }
                }
                else if p:Stage < Stage:Number
                {
                    // set spinStabLeadSecs to (burnTS - spinDCParam) - Time:Seconds.
                    if useNext
                    {
                        OutInfo("Setting preSpin").
                        set preSpinSecs to spinDCParam.
                    }
                    else
                    {
                        if spinDCParam >= predictedEngBurnTime
                        {
                            set preSpinSecs to Abs(predictedEngBurnTime - spinDCParam).
                        }
                    }
                }

                set spinCutoffTS to spinTS + spinStabDurSecs.
                if preSpinSecs > 0
                {
                    set spinTS to Round(burnTS - preSpinSecs, 2).
                    set preSpinArmed to true.
                }
                set g_SpinArmed to spinStabDurSecs > 0.
            }
            
            // Determine the overall lead time required to be ready at burnTS
            set burnLeadTime   to 15 + Max(burnUllageTime, preSpinSecs).
            set burnLeadTS     to burnTS - burnLeadTime.
            ...
            until time:seconds >= burnTS
            {
                ...
                if preSpinArmed
                {
                    if Time:Seconds >= spinTS
                    {
                        set Ship:Control:Roll to spinFactor.
                        OutInfo("Spin Started (T{0}) ":Format(Round(spinTS - Time:Seconds, 2)), 2).
                        set s_Val to _inNode:burnvector.
                    }
                    else if Time:Seconds >= spinCutoffTS
                    {
                        set Ship:Control:Roll to 0.
                        OutInfo("Arresting Spin                       ", 2).
                        set g_SpinArmed to false.
                    }
                    else if Time:Seconds  > 0
                    {
                        OutInfo("Spin Armed [{0}] (T-{1}) ":Format(Round(spinStabLeadSecs, 2), Round(spinTS - Time:Seconds, 2)), 2).
                        set s_Val to lookDirUp(_inNode:burnvector, rollUpVector:Call()).
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    else if g_TermChar = Char(82) // 'R'
                    {
                        ...
                        clr(cr()).
                        wait 0.1.
                        set burnDurations to CalcBurnDur(_inNode:deltaV:mag).
                        set fullBurnDuration to burnDurations[0].
                        set halfBurnDuration to burnDurations[3].
            ...
            local burnTimeRemaining to burnDurations[0].
            set t_Val to 1.
            lock throttle to t_Val.
            set s_Val to lookDirUp(_inNode:burnVector, rollUpVector:Call()).
            set Ship:Control:Fore to 0.
            ...
            {   
                ...
                {    
                    ...
                    if g_ActiveSpecs:Keys:Length > 0
                    {
                        ...
                        if g_ActiveSpecs:SpoolTime > 0.02
                        {
                            set softShutdownDV to dvRate * (g_ActiveSpecs:SpoolTime * 0.625).
                        }
                    }
                    
                    set burnTimeRemaining to GetBurnDurations(NextNode:DeltaV:Mag)[0].
                    local dvAcc to burnNode:deltaV:mag / maxAcc.
                    if dvAcc < 0.005
                    {
                        set t_Val to 0.
                        set breakFlag to true.
                    ...
                    {
                        set t_Val to choose burnNode:deltaV:mag / maxAcc if burnNode:deltaV:mag / maxAcc > 0.0125 else 0.
                        DispBurnNodeData(dv, burnTS - time:seconds, burnTimeRemaining).
                        DispBurnPerfData().
                    }
                    if ReacquireBurnVector
