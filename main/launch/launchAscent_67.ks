...
local tgtAlt0 to Ship:Body:SOIRadius.
local tgtAlt1 to 0.
local tgtInc to 30.
...
if tgtAlt1 = -1 
{
    set tgtAlt1 to Ship:Body:SOIRadius.
}
else if tgtAlt1 = 0
{
...
{   
    ...
    {
        OutMsg("TgtInc: {0} | TgtAlt: {1} | AscShaper: {2}":Format(tgtInc, tgtAlt0, ascShaper), g_line).
    ...
    if g_Program = 8
    {
        set g_AzData to l_az_calc_init(tgtAlt0, tgtInc).
...
local LESParts to Ship:PartsNamedPattern("(.*LES.*|.*Launch.*Escape.*)").
...
local ascAngDel to GetAscentAngle@:Bind(tgtAlt0):Bind(ascShaper).
...
until g_Program >= 36 or g_Abort
{
    ...
    else if g_Program = 22
    {
        if g_Runmode = 1
        {
            OutMsg("ROLL PROGRAM", g_TermHeight - 14).
            // set rollFlag to true.
            SetRunmode(2).
        }
        else if g_RunMode = 2
        {
            if Ship:Apoapsis >= tgtAlt0
            {
    ...
    else if g_Program = 24
    {
        if g_RunMode > 0
        {
            if Stage:Number = g_StageLimit
            {
                ...
                else if Ship:Periapsis >= tgtAlt1
                {
                    set g_AS_Armed to false.
                    SetProgram(30).
                }
    ...
    else if g_Program = 26
    {
        if g_RunMode > 0
        {
            if Ship:AvailableThrust <= 0.01 or Ship:Periapsis >= tgtAlt1 or Ship:Apoapsis >= (tgtAlt0 * 1.25)
            {
                clr(g_line).
                clr(cr()).
                RCS on.
    ...
    else if rollFlag
    {
        if VAng(Ship:Up:Vector, rollVec) <= 2.5
        {
            ...
            OutMsg("ROLL COMPLETE", g_TermHeight - 14).
        }
    }
