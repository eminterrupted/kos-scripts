...
if _params:length > 0 
{
  ...
  if _params:Length > 3 set retroBurnTS to _params[3]:ToNumber(retroBurnTS).
}
...
{
    SetStageLimit(g_MissionPlan:S[g_MissionPlan:S:Keys:Length - 1]:ToNumber(2)).
}
...
until g_Program > 199 or g_Abort
{
    ...
    else if g_Program = 40
    {
        ...
        {
            if Time:Seconds >= settleTS and VAng(Ship:Facing, Ship:SrfRetrograde) <= 0.25
            {
                set settleTS to 0.
                SetProgram(46).
            }
