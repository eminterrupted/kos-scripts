...
print "compareHashFile.ks" at (2, g_line).

if Exists(_inputPathA) and Exists(_inputPathB) {
    ...
    local rowAnchor to 25.
    local rowIncr to 10.
 
    set g_line to rowAnchor.


    for _file in list(_inputPathA, _inputPathB) {
        print "Byte count  : " at (0, g_line + 6). 
        print "Hash result : " at (0, g_line + 7). 
        
        print "Calculating hash(es) for file: {0}":Format(_file) at (0, cr()).
        local _ts to Time:Seconds.
        cr(g_line, 2).

        local inputHandle to Open(_file).
        local inputContent to inputHandle:ReadAll.
        local inputBinary to inputContent:Binary.

        from { local __line to g_line. local i to 0.} until i = selectedMethods:Keys:Length step { set i to i + 1. } do {
            
            local hashKey to selectedMethods:Keys[i].
            local activeHash to selectedMethods[hashKey].

            local curCol to colAnchor + (colIncr * i).
            print "{0,13} ":Format(activeHash[0]) at (curCol, __line).
            local _ts1 to Time:Seconds.
            local binaryHash to activeHash[1]:Call(inputBinary, _showProgress).
            local _ts2 to Time:Seconds. 
            if _showTimeStats {
                print "  ( {0,3} s )  ":Format(TimeSpan(Round(_ts2 - _ts1, 2)):seconds) at (curCol, __line + 1).
            }
            print "[{0,9}] ":Format(inputBinary:Length) at (curCol, __line + 3).
            print "[{0,9}] ":Format(binaryHash) at (curCol, __line + 4).

        }
        set _ts to Time:Seconds - _ts.
        // print "File Duration: {0} ":Format(TimeSpan(_ts):Full) at (0, 4).
...
print "script complete" at (2, g_line).
