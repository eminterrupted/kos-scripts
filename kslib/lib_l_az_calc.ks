...
global function l_az_calc_init {
    ...
    if abs(launchLatitude) > desiredInc {
        ...
        hudtext("Inclination impossible from current latitude, setting for lowest possible inclination.", 3, 3, 16, red, false).
    }.
    
    if 180 - abs(launchLatitude) < desiredInc {
        set desiredInc to 180 - abs(launchLatitude).
        hudtext("Inclination impossible from current latitude, setting for highest possible inclination.", 3, 3, 16, red, false).
    }.
